...
- A server where all models can run.
...
- **Frigate**
  [Frigate](https://github.com/blakeblackshear/frigate) is an event-based Network Video Recorder with low latency. It is composed from different moduls:
...
- **Double Take**
  ...
  - It takes the correct input from the correct topic list in a broker through HTTP calls. This is useful if we have more than one camera and we have more than one operation to do for each camera.
...
1. Input Multiplexing
   The *go2rtc* module of *Frigate* behaves as a proxy: it opens a single RTSP session with the camera and produces two TCP communications in order to avoid packet loss. The main-stream is a high-resolution channel with a high bitrate; the sub-stream is a low-resolution channel with a low bitrate. This allows leaving the other camera channel free for another client (e.g., live camera control from smartphone).
